...
// makeTokenMember issues a signed JWT carrying a linked member id.
func makeTokenMember(t *testing.T, userID, role, memberID string, ttl time.Duration) string {
	t.Helper()
	tok, err := auth.IssueAccessToken(userID, role, memberID, testSecret, ttl)
	if err != nil {
		t.Fatalf("makeTokenMember: %v", err)
	}
	return tok
}
