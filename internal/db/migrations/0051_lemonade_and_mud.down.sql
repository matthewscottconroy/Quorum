DELETE FROM arcade_plays WHERE game IN ('lemonade','sub-basement');
DELETE FROM arcade_scores WHERE game IN ('lemonade','sub-basement');
DELETE FROM arcade_levels WHERE game IN ('lemonade','sub-basement');
DELETE FROM arcade_player_stats WHERE game IN ('lemonade','sub-basement');
...
ALTER TABLE arcade_plays ADD CONSTRAINT arcade_plays_game_check CHECK (game IN
    ('chess','go','comet-buster','penny-pincher','brickfall','powder-keg','hexfection','interns','texas-holdem','red-tape','night-audit','lucky-penny','off-the-roof','pest-control','bumper-chairs','roll-call','floor-plan','homestead'));
