...
A basement arcade behind the **Top Secret** sidebar section: eight original cabinets written in Rust (Bevy engine, core crate only) and compiled to WebAssembly, served same-origin from `web/arcade/`. Credits are **play tokens, not money** — inserting one records a play against the user's account and never touches the ledger. Games: Chess (vs a built-in machine opponent, hotseat, or online — full draw rules: 50-move, threefold repetition, insufficient material; resign with R-R; online Black sees a flipped board), Go (hotseat or online, with last-move marker and hotseat undo), Comet Buster (now with a hunting saucer), Penny Pincher, Brickfall, Powder Keg and Hexfection (up to 12 seats each — local humans + bots, or online rooms; Powder Keg has a keg-kick perk and a post-sudden-death settle timer so rounds always end), and Interns (a walkers-and-jobs cabinet: assign seven jobs to a stream of clueless new hires and hit the rescue quota — 1P, 2P local with a keyboard cursor for the second player, 2P online, and a **level editor** whose creations are stored server-side and shared org-wide; levels run up to 1080 cells wide with a scrolling camera window, edge/A-D scrolling, and a minimap; hover highlights the walker your click would address, active jobs are reassignable, quitters keep moving through their countdown, undermined supervisors fall, and you can still hand out jobs while paused. The editor loads any level as a template, cycles widths, has per-skill pools, undo, a Shift-click line tool, and a mirror mode for symmetric 2P maps; finished test-plays return to the canvas without scoring). All cabinets have chip-tune sound (a WebAudio square-wave synth in the page — no assets, no autoplay issues since the coin click is the enabling gesture) and Esc pauses local rounds. The chess machine is a perft-verified negamax searcher in `arcade/logic` — beatable on purpose, but it finds short mates and refuses free-piece blunders (unit-tested for both).
