...
A basement arcade behind the **Top Secret** sidebar section: seven original cabinets written in Rust (Bevy engine, core crate only) and compiled to WebAssembly, served same-origin from `web/arcade/`. Credits are **play tokens, not money** — inserting one records a play against the user's account and never touches the ledger. Games: Chess (vs a built-in machine opponent, hotseat, or online — full draw rules: 50-move, threefold repetition, insufficient material; resign with R-R; online Black sees a flipped board), Go (hotseat or online, with last-move marker and hotseat undo), Comet Buster (now with a hunting saucer), Penny Pincher, Brickfall, Powder Keg and Hexfection (up to 12 seats each — local humans + bots, or online rooms; Powder Keg has a keg-kick perk and a post-sudden-death settle timer so rounds always end). All cabinets have chip-tune sound (a WebAudio square-wave synth in the page — no assets, no autoplay issues since the coin click is the enabling gesture) and Esc pauses local rounds. The chess machine is a perft-verified negamax searcher in `arcade/logic` — beatable on purpose, but it finds short mates and refuses free-piece blunders (unit-tested for both).
